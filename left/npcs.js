...
let year = 0;
const initialLifespan = 1000;
const initialMembers = 450;
...
Object.keys(houses).forEach(houseName => {
    ...
    for (let i = 0; i < initialMembers; i++) {
        ...
        let elf = {
            ...
            cuibri: initialLifespan, //gas in the tank
            lanwa: initialLifespan   // size of the tank
        }
...
function qq(yearsPass = 1) {
    
    const mortalLifeSpan = 90; 
    yearsPass = parseInt(yearsPass);
    ...
    // Simulation loop
    for (i=0; i < yearsPass; i++) {
       
    //E.1. Calculate Full Elf Births 
    let eBirthRate = Math.random() * (9.9 - 0.9) + 0.9;
        EBirthRate.push(eBirthRate);
    let eParents = Math.floor(elves.filter(elf => elf.cuibri > 35 && elf.loa >= 45).length/2);
    let eBirths = Math.ceil((eBirthRate/100) * eParents);
    ...
    for (let i = 0; i < eBirths; i++) {
        ...
        let elf = {
            parents: "elves",
            loa: 0, //loa = age.
            cuibri: mortalLifeSpan, //gas in the tank
            lanwa:  mortalLifeSpan, // size of the tank
        }
    ...
    //E.3. Kill Elves
     let eDeaths = elves.filter(elf => elf.cuibri <= 0);
         totalEDeaths += eDeaths.length;
     
         elves = elves.filter(elf => elf.cuibri > 0);
     
    
    
    //H.1. Calculate Human Births 
    let hBirthrate = Math.random() * (9.9 - 0.9) + 0.9;
        HBirthRate.push(hBirthrate);
    let hParents = Math.floor((humans.filter(human => human.loa >= 16 && human.loa <= 35).length)/2);
    let hBirths = 0 //Math.ceil((hBirthrate/100) * hParents);
    ...
    //H.3. Kill Humans
    let hDeaths = humans.filter(human => human.cuibri <= 0).length;
    totalHDeaths += hDeaths;
    humans = humans.filter(human => human.cuibri > 0);   

    }

    let totalECuibri = elves.reduce((acc, currentElf) => {
    return acc + currentElf.cuibri;
    }, 0);

    const EBirthRateSum = EBirthRate.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
    const EBirthRateMean = EBirthRateSum / EBirthRate.length;

    let totalHCuibri = elves.reduce((acc, currenHum) => {
    return acc + currenHum.cuibri;
    }, 0);

    const HBirthRateSum = HBirthRate.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
    const HBirthRateMean = HBirthRateSum / HBirthRate.length;

  

    let totalCuibri = totalECuibri + totalHCuibri

      
      // 4. Store Results
      results.push({
        // House: houseName,
        Missing: (totalHBirths + totalEBirths) - ((totalHDeaths + totalEDeaths) + humans.length),
        Year: year + yearsPass,
        Cuibri: totalCuibri,
        Cuirbar: {
            Births: totalEBirths,
            BirthRate: EBirthRateMean.toFixed(2),
            Deaths: totalEDeaths,
            Children: elves.filter(elf => elf.loa < 32).length,
            Fertile: elves.filter(elf => elf.loa >= 45 && elf.loa <= 55).length,
            Total:  elves.length
        },
        Mortal: {
            Births: totalHBirths,
            BirthRate: HBirthRateMean.toFixed(2),
            Deaths: totalHDeaths,
            Babies: humans.filter(human => human.loa === 1).length,
            Children: humans.filter(human => human.loa > 1 && human.loa < 16).length,
            Fertile: humans.filter(human => human.loa >= 16 && human.loa <= 35).length,
            Adults: humans.filter(human => human.loa > 35 && human.load < 65).length,
            Elders: humans.filter(human => human.loa > 65).length,
            Total: humans.length,
      ...
      console.log('humans: ' + humans.filter(human => human.parents === "humans").length)
      console.log('elves: ' + humans.filter(human => human.parents === "elves").length)

})

    year += yearsPass

    //currentYear = currentYear + yearsToSimulate
